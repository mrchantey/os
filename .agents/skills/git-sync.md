...
- **Genuine value clash — stop and ask.** Reserve stopping for conflicts where the *meaning* diverges, not just the wording: the same key/setting given two *different actual values* (a keybind → two different keys, a number/path/colour that disagrees), overlapping edits where each side rewrites or deletes the other's logic, or reordering where order is load-bearing (sourced configs, `PATH`, etc.). Also **always stop** for the session-critical hypr files (`monitors.conf`, `input.conf`, `hyprland.conf`, `hypridle.conf`, `layout.conf`) even when it looks trivial — a bad merge there can wedge the session. Report the conflicting files + the diff, let the user resolve, then `git rebase --continue` (or `git rebase --abort` to back out).
