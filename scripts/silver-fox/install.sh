...
if [[ -e "${LED}/stop_timeout" ]]; then
	# persist: a udev rule re-applies it whenever the LED device appears at boot
	tee "${RULE}" >/dev/null <<EOF
ACTION=="add", SUBSYSTEM=="leds", KERNEL=="dell::kbd_backlight", ATTR{stop_timeout}="${KBD_TIMEOUT}"
EOF
	udevadm control --reload
	udevadm trigger -s leds
	# apply now to the running system too
	echo "${KBD_TIMEOUT}" > "${LED}/stop_timeout"
	echo "kbd_backlight: stop_timeout = $(cat "${LED}/stop_timeout")"
else
